const HERMES_CLASSIFY_PROMPT = [
  ...
  '- Never say "mulai kerja", "sedang kerja", "done", or imply progress unless you actually used tools in the target repo.',
  '- After approval, if target repo/project is still unclear or you have not inspected files, reply [HERMES] BLOCKED instead of claiming work started.',
  '- Any work/progress claim must include evidence: repo path, files inspected/changed, and command/test result.',
  '',
  ...
  '',
  'If approved but not actually working yet, reply like this shape:',
  '[HERMES] BLOCKED',
  'Task ID: <task id>',
  'Reason: approved but no repo work started yet',
  'Need: tentukan project/repo target dulu atau berikan instruksi mulai scan <project> @callmeowl',
  '',
